...
public class AddressBookManager {
	...
	public void removeContactObserverHandler(Handler handler) {
		if (_mContactsContentObserver != null) {
			_mContactsContentObserver.removeHandler(handler);
		}
	}
	...
	public void registContactOberver() {
		...
		_mContentResolver.registerContentObserver(Contacts.CONTENT_URI, true,
				_mContactsContentObserver);
	}

	public void unRegistContactObserver() {
		if (_mContactsContentObserver != null) {
			_mContentResolver
					.unregisterContentObserver(_mContactsContentObserver);
		}
	}
	...
	public List<ContactBean> getAllNamePhoneticSortedContactsInfoArray() {
		// Log.d("AddressBook", "get all contact form addressbook to contact " +
		// _mAllContactsInfoArray.size());
	...
	public void copyAllContactsInfo(List<ContactBean> oldContactsInfo) {
		...
		Collections.sort(oldContactsInfo, CONTACTNAMEPHONETIC_COMPARATOR);
	...
	public void updateContactPhone(long contactId, String oldPhone,
			String newPhone) {
		...
		Cursor phoneCursor = _mContentResolver.query(Data.CONTENT_URI,
				projection, selection, selectionArgs, null);
		if (phoneCursor != null) {
			while (phoneCursor.moveToNext()) {
				long id = phoneCursor.getLong(phoneCursor
						.getColumnIndex(Phone._ID));
				long r_id = phoneCursor.getLong(phoneCursor
				...
				if (phone.equals(oldPhone)) {
					...
					String where = Phone.RAW_CONTACT_ID + "=? and "
							+ Data.MIMETYPE + "=? and " + Phone._ID + "=?";
					String[] args = { r_id + "", Phone.CONTENT_ITEM_TYPE,
							id + "" };
					_mContentResolver.update(Data.CONTENT_URI, contentValue,
							where, args);
					break;
				}
			}
	...
	public int syncContact() {
		...
		for (ContactBean _contact : _mAllContactsInfoArray) {
			...
			if (ContactDirtyType.NORMAL == _contact.getDirty()) {
				...
				if (_deletedContact) {
					Log.d(LOG_TAG,
							"the contact aggregated id = " + _contact.getId()
									+ " deleted");
		...
		for (Long _dirtyContactId : _allDirtyContactsIdsList) {
			...
			final String[] _dataProjection = new String[] { Data.MIMETYPE,
					...
					StructuredName.GIVEN_NAME, StructuredName.MIDDLE_NAME,
					StructuredName.FAMILY_NAME, Phone.NUMBER, Photo.PHOTO };
			...
			Cursor _dataCursor = _mContentResolver.query(Data.CONTENT_URI,
					_dataProjection, _dataSelection, _dataSelectionArgs, null);
			...
			if (null != _dataCursor) {
				while (_dataCursor.moveToNext()) {
					...
					if (_mAllContactsInfoMap.containsKey(_dirtyContactId)) {
						// get the contact
						_contact = _mAllContactsInfoMap.get(_dirtyContactId);
			...
			if (_contact != null) {
				...
				// _dirtyContactId is deleted! so just delete it from map and
				// array
				ContactBean deletedContact = _mAllContactsInfoMap
						.get(_dirtyContactId);
		...
		if (_allDirtyContactsIdsList.size() != 0) {
			// if contacts change all the search result cache should be cleared
	...
	class ContactsContentObserver extends ContentObserver {
		private Set<Handler> handlerList;
		...
		public void onChange(boolean selfChange) {
			int updateType = syncContact();

			for (Handler handler : handlerList) {
				Message m = handler.obtainMessage(updateType, "updateABList");
